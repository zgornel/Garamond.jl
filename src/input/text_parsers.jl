...
The input_parser is generated by `parse_input` and its signature should be of the form:
    ...
    some_input_parser(query, dbschema [;separator=DEFAULT_QUERY_PARSING_SEPARATOR, searchable_filters=[])

where:
    `query` is the input query
    `dbschema` is db schema of the data
...
function parse_input(env, request)
    ...
    return input_parser(request.query,
...
function base_input_parser(query,
                           dbschema;
...
function noop_input_parser(query, args...; kwargs...)
    search_query = query
    filter_query = Dict{Symbol, Any}()
    return (search=search_query, filter=filter_query)
end
