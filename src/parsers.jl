...
function get_parser_function(pname::AbstractString)
	if pname == "cornel_csv"
		config = CSVParserConfig(
			Dict(1=>:id, 2=>:author, 3=>:name, 4=>:publisher, 
      				5=>:edition_year, 6=>:published_year, 8=>:documenttype),
...
# Function to load corpora using a Garamond data config file
function load_corpora(dataconfigpath::AbstractString)
	crefs = parse_data_config(dataconfigpath)
	load_corpora(crefs)
end

# Function to load corpora using a vector of corpus references
function load_corpora(crefs::Vector{CorpusRef})
	
	crpra = Corpora()
	
	# Loop over each corpus reference 
	for cref in crefs
